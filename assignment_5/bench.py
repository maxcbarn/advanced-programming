...
def main():
    dataTime = pd.read_csv("./logs/TimeLog.csv", names=['Size', 'PathQuantity', 'ExecutionTime', 'Dist', 'PathBlocked' ])
    dataTimeGrupped = dataTime.groupby(['Size', 'PathQuantity'])['ExecutionTime'].mean().reset_index()
    df40 = dataTimeGrupped[ dataTimeGrupped['Size'] == 40 ]
    df40 = df40.groupby('PathQuantity')['ExecutionTime'].mean().reset_index()

    TimeXQuantity( dataTimeGrupped )

    dataObstacle = pd.read_csv("./logs/ObstacleLog.csv", names=[ 'PathQuantity', 'ObstacleQuantity' , 'ExecutionTime', 'Dist', 'PathBlocked' ])
    dataObstacleGrupped = dataObstacle.groupby(['ObstacleQuantity', 'PathQuantity'])['ExecutionTime'].mean().reset_index()

    ObstacleXTime( dataObstacleGrupped , df40 )
    
    dataDist = pd.read_csv("./logs/DistLog.csv", names=[ 'PathQuantity', 'ExecutionTime', 'Dist', 'PathBlocked' ])
    dataDistGrupped = dataDist.groupby('Dist')['ExecutionTime'].mean().reset_index()
    
    TimeXDist( dataDistGrupped )
