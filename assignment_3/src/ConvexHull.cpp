...
ConvexHull::ConvexHull( std::vector< Vector2 > points , Sides * sides , Constrains * constrains , std::string& log ) {
    if( points.size() < 3 ) {
        return;
    }
    ...
    log += std::to_string( points.size() );
    log += ',';
    log += std::to_string( executionTime.count() );
    log += ',';
    log += std::to_string( upper.size() );
    log +=',';
    log += std::to_string( points.size() - upper.size() );
}
